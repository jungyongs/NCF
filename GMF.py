...
def get_model(num_users, num_items, latent_dim, regs=0):
    # Input layers
    user_input = Input(shape=(1,), dtype='int32', name = 'user_input')
    item_input = Input(shape=(1,), dtype='int32', name = 'item_input')

    # Embedding layers
    Embedding_User = Embedding(input_dim = num_users, output_dim = latent_dim, name = 'user_embedding',
                                  embeddings_initializer = random_normal, embeddings_regularizer = l2(regs), input_length=1)
    Embedding_Item = Embedding(input_dim = num_items, output_dim = latent_dim, name = 'item_embedding',
                                  embeddings_initializer = random_normal, embeddings_regularizer = l2(regs), input_length=1)   
    ...
    # Output Layer
    output = Dense(1, activation = 'sigmoid', kernel_initializer='lecun_uniform', name = 'output')(predict_vector)

    model = Model(inputs=[user_input, item_input], outputs=output, name = 'GMF')
